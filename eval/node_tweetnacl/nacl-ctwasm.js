(nacl => {
	...
	const fs = require('fs');
	const wasmCode = fs.readFileSync(__dirname + '/tn_ctwasm.wasm');
	const wasmCodePublicModule = fs.readFileSync(__dirname + '/tn_ctwasm_sign_open.wasm');
	...
	let wasmMemory = new WebAssembly.Memory({initial: 2000, secret: true});
	...
	*/
	...
	(() => {
		...
		wasmInstancePromise = WebAssembly.instantiate(wasmCode, {js: {mem: wasmMemory}});
		wasmInstancePromise
			.then(result => {
